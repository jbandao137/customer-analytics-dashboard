...
- Predicción de churn por lotes (CSV del usuario)
...
# When launched as `streamlit run src/dashboard.py`, Streamlit puts src/ on
# sys.path instead of the project root, breaking `from src.X import ...`.
# This line ensures the project root is always resolvable regardless of how
# the script is invoked (streamlit CLI, Streamlit Cloud, or test imports).
_root = Path(__file__).resolve().parent.parent
if str(_root) not in sys.path:
    sys.path.insert(0, str(_root))

from src.train_model import validar_columnas, predecir_por_lotes, REQUIRED_FEATURES
...
def _generar_plantilla() -> bytes:
    """CSV de muestra con las columnas requeridas y tres filas de ejemplo."""
    datos = {
        "genero":              ["Masculino",          "Femenino",              "Masculino"],
        "tiene_pareja":        ["Si",                 "No",                    "Si"],
        "tiene_dependientes":  ["No",                 "Si",                    "No"],
        "servicio_telefono":   ["Si",                 "Si",                    "No"],
        "servicio_internet":   ["Fibra optica",       "DSL",                   "No"],
        "soporte_tecnico":     ["No",                 "Si",                    "No"],
        "tipo_contrato":       ["Mensual",            "Anual",                 "Bianual"],
        "metodo_pago":         ["Cheque electronico", "Transferencia bancaria", "Cheque por correo"],
        "factura_electronica": ["Si",                 "No",                    "Si"],
        "edad":                [35,                   52,                      28],
        "es_senior":           [0,                    0,                       0],
        "meses_antiguedad":    [6,                    36,                      60],
        "cargo_mensual":       [89.50,                55.00,                   32.75],
        "cargo_total":         [537.0,                1980.0,                  1965.0],
        "tickets_soporte":     [4,                    1,                       0],
    }
    return pd.DataFrame(datos).to_csv(index=False).encode("utf-8")
...
# ----------------------------- Pestañas ------------------------------------
tab1, tab2 = st.tabs(["📊 Análisis del dataset", "📂 Predicción por lotes"])
...
# ===========================================================================
    ...
    st.subheader("📂 Predicción de churn por lotes")
    st.markdown(
        "Carga un CSV con los datos de tus clientes y el modelo calculará "
        "la probabilidad de abandono para cada uno."
    )
    ...
    with col_up:
        archivo = st.file_uploader(
            "Selecciona un archivo CSV de clientes",
            type=["csv"],
            help="El CSV debe incluir las columnas de la plantilla de ejemplo.",
        )

    with col_tmpl:
        st.markdown("**¿Primera vez?**")
        st.markdown("Descarga el CSV con las columnas requeridas y filas de muestra.")
        st.download_button(
    ...
    if archivo is not None:
        ...
        if df_lote is not None:
            if df_lote.empty:
                st.error("El archivo está vacío. Usa la plantilla como referencia.")
            else:
                valido, faltantes = validar_columnas(df_lote)
                if not valido:
                    st.error(
                    ...
                    st.warning(
                        ...
                        "Ejecuta: `python src/train_model.py`"
                    )
                else:
                    try:
                        resultado = predecir_por_lotes(modelo, df_lote)

                        n_total = len(resultado)
                        n_alto  = int((resultado["prob_churn"] > 0.5).sum())
                        n_medio = int(
                            ((resultado["prob_churn"] > 0.3) &
                             (resultado["prob_churn"] <= 0.5)).sum()
                        )

                        m1, m2, m3 = st.columns(3)
                        m1.metric("Total clientes", f"{n_total:,}")
                        m2.metric("Alto riesgo (> 50%)", f"{n_alto:,}",
                                  f"{n_alto / n_total:.1%} del total")
                        m3.metric("Riesgo medio (30–50%)", f"{n_medio:,}",
                                  f"{n_medio / n_total:.1%} del total")

                        st.divider()
                        st.markdown("**Resultados ordenados por probabilidad de churn ↓**")

                        df_vista = resultado.copy()
                        df_vista.insert(
                            0, "Nivel de riesgo",
                            df_vista["prob_churn"].apply(
                                lambda p: "🔴 Alto" if p > 0.5
                                else ("🟡 Medio" if p > 0.3 else "🟢 Bajo")
                            ),
                        )
                        df_vista["prob_churn"] = df_vista["prob_churn"].apply(
                            lambda p: f"{p:.1%}"
                        )
                        df_vista = df_vista.rename(columns={"prob_churn": "Prob. churn"})
